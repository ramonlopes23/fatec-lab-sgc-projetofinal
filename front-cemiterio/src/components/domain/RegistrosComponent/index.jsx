...
const getSepultamentoIdFromExumacao = (exumacao) =>
    normalizeId(exumacao?.sepultamentoId ?? exumacao?.sepultamento_id ?? exumacao?.sepultamento);
...
export default function RegistrosComponent() {
    ...
    const loadAll = useCallback(async () => {
        ...
        try {
            ...
            sepultamentosData.forEach((sepultamento) => {
                ...
                if (falecidoId && !sepultamentosByFalecido.has(falecidoId))
                    sepultamentosByFalecido.set(falecidoId, sepultamento);
            });
            ...
            exumacoesData.forEach((exumacao) => {
                ...
                if (sepultamentoId && !exumacoesBySepultamento.has(sepultamentoId))
                    exumacoesBySepultamento.set(sepultamentoId, exumacao);
            });
    ...
    const ageSeries = useMemo(() => {
        ...
        return AGE_RANGE_ORDER.map((key) => ({
            key,
            label: ageRangeLabel(key),
            value: totals[key] || 0,
            color: AGE_RANGE_COLORS[key],
        })).filter((item) => item.value > 0);
    }, [filteredRecords]);

    const cemeteryOptions = useMemo(() => {
        const values = Array.from(
            new Set(records.map((item) => String(getCemiterio(item)).trim()).filter((value) => value && value !== "-"))
        );
        return values.sort((left, right) => left.localeCompare(right, "pt-BR", { sensitivity: "base" }));
    }, [records]);

    const quadraOptions = useMemo(() => {
        const base = filters.cemiterio
            ? records.filter((item) => String(getCemiterio(item)) === String(filters.cemiterio))
            : records;
    ...
    const modalViewFields = modalFields.map(([label, key, type]) => [label, getModalFieldValue(key, type)]);
            ...
            <Container>
                <PageHeader>
                    <HeaderCopy>
                        ...
                        <Subtitle>
                            Acompanhe os dados cadastrais dos falecidos, sua situação e localização nos cemitérios.
                        </Subtitle>
                    </HeaderCopy>
                    <PeriodChip>
                        <PeriodChipLabel>Base ativa</PeriodChipLabel>
                        <PeriodChipValue>
                            {records.length.toLocaleString("pt-BR")} falecidos cadastrados
                        </PeriodChipValue>
                        <PeriodChipValue>
                            {filteredRecords.length.toLocaleString("pt-BR")} registros filtrados
                        </PeriodChipValue>
                    </PeriodChip>
                </PageHeader>
                    ...
                    <FilterGrid>
                        ...
                        <FilterRow>
                            <FormControl fullWidth size="medium">
                                ...
                                <Select
                                    ...
                                    label="Faixa etária"
                                    onChange={updateFilter("faixaEtaria")}
                                    sx={filterSelectSx}
                                >
                            ...
                            <FormControl fullWidth size="medium">
                                ...
                                <Select
                                    ...
                                    label="Situacao"
                                    onChange={updateFilter("situacao")}
                                    sx={filterSelectSx}
                                >
                                    <MenuItem value="">Todas</MenuItem>
                        ...
                        <FilterRow>
                            <FormControl fullWidth size="medium">
                                ...
                                <Select
                                    value={filters.cemiterio}
                                    label="Cemiterio"
                                    onChange={updateFilter("cemiterio")}
                                    sx={filterSelectSx}
                                >
                                    <MenuItem value="">Todos</MenuItem>
                                    {cemeteryOptions.map((cemiterio) => (
                                        <MenuItem key={cemiterio} value={cemiterio}>
                                            {cemiterio}
                                        </MenuItem>
                                    ))}
                                </Select>
                        ...
                        <FilterRow>
                            <FormControl fullWidth size="medium">
                                ...
                                <Select
                                    ...
                                    label="Sepultura"
                                    onChange={updateFilter("sepultura")}
                                    sx={filterSelectSx}
                                >
                                    <MenuItem value="">Todas</MenuItem>
                                    ...
                                    pageRecords.map((record, index) => {
                                        ...
                                        return (
                                            <Tr key={record.id || `${getFalecidoNome(record)}-${index}`} $index={index}>
                                                ...
                                                <Td>
                                                    <StatusBadge $tone={situacaoTone(situacao)}>
                ...
                <DefaultModal
                    ...
                    subtitle={
                        isEditing
                            ? "Edite os dados principais do registro."
                            : "Visualização completa do registro cadastral."
                    }

...
export const StatusBadge = styled.span`
  ...
  ${({ $status }) => {
    ...
    if (s === "interditado") return "#b45309";
    if (s === "manutencao") return "#64748b";
    return "#6b7280";
  }};
  color: #fff;
  background: ${({ $status }) => {
    const s = String($status || "").trim().toLowerCase();
    if (s === "disponivel") return "linear-gradient(135deg, #22c55e 0%, #16a34a 100%)";
    if (s === "ocupado") return "linear-gradient(135deg, #ef4444 0%, #dc2626 100%)";
    if (s === "interditado") return "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)";
    if (s === "manutencao") return "linear-gradient(135deg, #64748b 0%, #475569 100%)";
    return "#6b7280";
  }};

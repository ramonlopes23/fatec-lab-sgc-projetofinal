...
export default function NotificationsDropdown() {
    ...
    const notifications = useMemo(() => {
        ...
        const exumacaoItems = sepultamentos
            .map((sepultamento) => buildExumacaoNotification(sepultamento, falecidos))
            .filter(Boolean);

        return [...contractItems, ...exumacaoItems].sort((a, b) => a.daysLeft - b.daysLeft).slice(0, 5);
    }, [contratos, falecidos, sepultamentos]);

    const totalCount = useMemo(() => {
        const contractCount = contratos.map(buildContractNotification).filter(Boolean).length;
        const exumacaoCount = sepultamentos
            .map((sepultamento) => buildExumacaoNotification(sepultamento, falecidos))
                                ...
                                <NotificationItem key={item.id}>
                                    ...
                                    <NotificationContent>
                                        <NotificationTopRow>
                                            ...
                                            <NotificationBadge $bg={`${item.color}14`} $color={item.color}>
                                                {item.badgeLabel}
                                            </NotificationBadge>
                                        </NotificationTopRow>

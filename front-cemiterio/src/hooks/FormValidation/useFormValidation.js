...
export default function useFormValidation(form, processType, isIndigente, searchFal) {
    ...
    const validateFieldOnChange = useCallback(
        (fieldName, value) => {
            ...
            let rule =
                processType === PROCESS_TYPES.falecido ? RULES_FALECIDO[fieldName] : RULES_SEPULTAMENTO[fieldName];
    ...
    const validateBeforeSubmit = useCallback(() => {
        ...
        if (processType === PROCESS_TYPES.falecido) {
            ...
            if (
                !isEmpty(form.data_nasc) &&
                !isEmpty(form.dh_falec) &&
                !isValidDateRange(form.data_nasc, form.dh_falec)
            ) {
        ...
        if (processType === PROCESS_TYPES.sepultamento) {
            ...
            if (form?.com_velorio) {
                ...
                if (
                    !isEmpty(form.dh_inicio_velorio) &&
                    !isEmpty(form.dh_fim_velorio) &&
                    !isValidDateRange(form.dh_inicio_velorio, form.dh_fim_velorio)
                ) {

...
import {
    ...
    isSepulturaPerpetual,
} from "../../utils/sepultura.js";
...
export default function useAvailableCovas(covas, form, setForm) {
    const computeAvailableCovas = useCallback((covasList, quadraId, tituloPosse) => {
        if (!quadraId) return [];
        const sameQuadra = covasList.filter((cova) => getSepulturaQuadraRef(cova) === String(quadraId));
    ...
    const tipoCovaSelecionada = useMemo(() => {
        ...
        const byNumber = (list) => list.find((cova) => getSepulturaNumber(cova) === target);
        const found =
            byNumber(availableCovas) ||
            covas.find(
                (cova) => getSepulturaQuadraRef(cova) === String(form.quadra_sep) && getSepulturaNumber(cova) === target
            );
        if (!found) return "";
        return getSepulturaType(found);
    }, [availableCovas, covas, form.quadra_sep, form.num_sepultura_sep]);

    const handleQuadraSepChange = useCallback(
        (val) => {
